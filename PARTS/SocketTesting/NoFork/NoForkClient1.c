...
int main(void){
    ...
    /*  TO DO LIST                         | DONE
        ...
        * Once I'm sure that works,        |  
          use the capsule_booking          |     
          function. Not certain            |   
          that capsule_booking()           |       
        ...
        if(sockfd < 0){
            ...
            *p8 = SYNC_REQ;
            ...
            if(n == -1){
                ...
                // Get two bytes (packet count), read them into buffer16.
                while(totalsent < packetsize){
                    n = recv(sockfd, p8 + totalsent, bytesleft, 0);
                    if(n == -1){
                        perror("recv1");
                        break;
                    }
                ...
                if(n == -1){
                    ...
                    for(i = 0; i < count; i++){
                        ...
                        bytesleft = packetsize;
                         p8 = (uint8_t *)&buffer16;
                        // Get two bytes (packet size), read them into buffer16.
                        while(totalsent < packetsize){
                            n = recv(sockfd, p8 + totalsent, bytesleft, 0);
                            if(n == -1){
                                perror("recv1");
                        ...
                        if(n == -1){
                            printf("\nError receiving packet %d's first two bytes.", i);
                        }
                        else{
                            printf("\nPacket %d:", i + 1);
                            // Convert 2 bytes to a host 
...
//----------------------------------------------------------------------------
    do{
        ...
        if(ch == ADD){
            ...
            // Capsulise booking.            
            /* if((packet_size = capsule_booking(&packet, &booking_buffer)) < 0){
            ...
            *p8 = booking_buffer.struct_id; 
            ...
            if(sockfd < 0){
                ...
                // Send packet.
                uint16_t totalsent = 0;
                uint16_t bytesleft = packet_size + 2; 
                int n;
        ...
        //---------------------------------------------------------------------
            ...
            /*
              Ask user for id of booking he wants to edit.
              Search list for this id. If list empty, say list empty.
              If id not found, say not found.
              If found, copy booking into buffer.
              go through booking with user, if user presses enter,
              keep field as it is. if user enters a string, replace string
              in buffer with what user enters. If string requires resizing, do it.
              After this, send it to server with EDIT_BOOKING type.
              If server successful, update local copy. (delete and append new one.)
            */
            ...
            if(root == NULL)
                ...
                if(scout == NULL)
                    ...
                    if(scout == NULL)
                        ...
                        if(*str_buff != '\0'){
                            // Id user entered a new value, measure it's length first.
                            ...
                            // If .start needs to be resized, do it here.
                            // I need to completely rewrite this.
                            // Just make a new thing and replace the old one.
                        ...
                        *p32 = htonl(*p32);
                        ...
                        if(sockfd < 0){
                            ...
                            if(n == -1){
                                ...
                                // Now that booking is different, 
                                // I can delete the old one and append the new one.
...
uint16_t capsule_booking(uint8_t **data, struct _booking *booking){
    ...
    uint8_t *p8, *packet = *data;
    uint16_t *p16;
    uint32_t *p32;
    
    // data size does not include the size variable itself.
    uint16_t packet_size = 1 + 4 + strlen(booking->start) +
                           strlen(booking->end) + strlen(booking->name) +
                           strlen(booking->phone) + strlen(booking->account_number)
                           + strlen(booking->email) + strlen(booking->info) +
                           strlen(booking->time) + strlen(booking->flight_number)
                           + 9;
